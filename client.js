...
Object.keys(client.CalculatorService.CalculatorPort)); 
    ...
    console.log("Modulo:", mod[0].result);
    const power = await client.PowerAsync({ a: 2, b: 3 });
    console.log("Power:", power[0].result);
    const ctof = await client.CelsiusToFahrenheitAsync({ a: 30 });
    console.log("CelsiusToFahrenheit:", ctof[0].result);

    const ftoc = await client.FahrenheitToCelsiusAsync({ a: 86 });
    console.log("FahrenheitToCelsius:", ftoc[0].result);

    const ctok = await client.CelsiusToKelvinAsync({ a: 25 });
    console.log("CelsiusToKelvin:", ctok[0].result);
